...
## Rates
mu = 6 # clock-like mut rate (known)
...
m_clock = omega*mu*(t_mrca-t_eca)
m_alpha = omega*(mu*(t_cna - t_eca) +
                   mu_th_1*(t_cna - t_th_1_start))
m_beta = n*omega*(mu*(t_mrca-t_th_1_end) +
                    mu_th_1*(t_th_1_end-t_cna))
m_th_1 = omega*mu_th_1*(t_th_1_end - t_th_1_start)
...
convert_real_date = function(date = NULL, ref_year = 2000) {
  ...
  return((year - ref_year) + (month / 12 - ref_month / 12) + (day / 365 -
                                                                ref_day / 365))
  # return( (year-ref_year) + (month/12 - ref_month/12))
}
...
exampleData = list(
  ...
  'Parameters' =
    data.frame('Param name' = c('N_max','N_min','mu','omega_alpha','omega_beta', 'alpha_mu_th_1','beta_mu_th_1', 'k'),
