...
parameters{
  ...
  vector<lower=t_eca, upper=t_mrca>[n_cna] t_cna;
...
model{
  ...
  for (cna in 1:n_cna){
    t_cna[cna] ~ uniform(t_eca, t_mrca)
  }
  ...
  // clock-like
  m_clock ~ poisson(2*l_diploid*omega*(mu_clock*(t_mrca-t_eca)));
  vector[n_cna] lambda_alpha_clock;
  vector[n_cna] lambda_beta_clock;
  for (cna in 1:n_cna){
    lambda_alpha_clock[cna] = mu_clock*(t_cna[cna]-t_eca);
    lambda_beta_clock[cna] = mu_clock*(t_mrca-t_cna[cna]);
  }
  ...
  for (m in 1:n_th_step_type) {
    ...
    vector[n_cna] lambda_alpha_th_step;
    vector[n_cna] lambda_beta_th_step;
    for (cna in 1:n_cna){
      lambda_alpha_th_step[cna] = mu_th_step[m] * lambda_therapy(size(idx_step[m]), t_eca, t_cna[cna], t_start_m, t_end_m, k_step));
      lambda_beta_th_step[cna] = mu_th_step[m] * lambda_therapy(size(idx_step[m]), t_cna[cna], t_mrca, t_start_m, t_end_m, k_step));
    }
  }
  ...
  for (c in 1:n_th_cauchy_type) {
    ...
    vector[n_cna] lambda_alpha_th_cauchy;
    vector[n_cna] lambda_beta_th_cauchy;
    for (cna in 1:n_cna){
      lambda_alpha_th_cauchy[cna] = mu_clock * couchy_cdf(size(idx_cauchy[c]), location_c, scales_th_cauchy[c], t_eca, t_cna[cna]);
      lambda_beta_th_cauchy[cna] = mu_clock * couchy_cdf(size(idx_cauchy[c]), location_c, scales_th_cauchy[c], t_cna[cna], t_mrca);
    }
  }

  // alpha and beta
  for (cna in 1:n_cna){
    m_alpha[cna] ~ poisson(l_CNA[cna]*omega*(lambda_alpha_clock + lambda_alpha_th_step + lambda_alpha_th_cauchy));
    m_beta[cna] ~ poisson(l_CNA[cna]*omega*coeff*(lambda_beta_clock + lambda_beta_th_step + lambda_beta_th_cauchy));
  }
...
generated quantities{
  ...
  // clock-like
  int m_clock_rep = poisson_rng(2*l_diploid*omega*(mu_clock*(t_mrca-t_eca)));
  vector[n_cna] lambda_alpha_clock_rep;
  vector[n_cna] lambda_beta_clock_rep;
  for (cna in 1:n_cna){
    lambda_alpha_clock_rep[cna] = mu_clock*(t_cna[cna]-t_eca);
    lambda_beta_clock_rep[cna] = mu_clock*(t_mrca-t_cna[cna]);
  }

  // therapies
  vector[n_th_step_type] m_th_step_rep;
  for (m in 1:n_th_step_type) {
    vector[size(idx_step[m])] t_start_m_rep;
    vector[size(idx_step[m])] t_end_m_rep;

    for (j in 1:size(idx_step[m])) {
      t_start_m_rep[j] = t_therapy_i[idx_step[m][j]];
      t_end_m_rep[j] = t_therapy_f[idx_step[m][j]];
    }

    m_th_step_rep[m] = poisson_rng(2 * l_diploid * omega * mu_th_step[m] *
                         lambda_therapy(size(idx_step[m]), t_eca, t_mrca, t_start_m_rep, t_end_m_rep, k_step));
    vector[n_cna] lambda_alpha_th_step_rep;
    vector[n_cna] lambda_beta_th_step_rep;
    for (cna in 1:n_cna){
      lambda_alpha_th_step_rep[cna] = mu_th_step[m] * lambda_therapy(size(idx_step[m]), t_eca, t_cna[cna], t_start_m, t_end_m, k_step));
      lambda_beta_th_step_rep[cna] = mu_th_step[m] * lambda_therapy(size(idx_step[m]), t_cna[cna], t_mrca, t_start_m, t_end_m, k_step));
    }
  }

  vector[n_th_cauchy_type] m_th_cauchy_rep;
  for (c in 1:n_th_cauchy_type) {
    vector[size(idx_cauchy[c])] location_c_rep;

    for (j in 1:size(idx_cauchy[c])) {
      location_c_rep[j] = location_th_cauchy[idx_cauchy[c][j]];
    }

    m_th_cauchy_rep[c] = poisson_rng(2 * l_diploid * omega * mu_clock *
                           couchy_cdf(size(idx_cauchy[c]), location_c, scales_th_cauchy[c], t_eca, t_mrca));

    vector[n_cna] lambda_alpha_th_cauchy_rep;
    vector[n_cna] lambda_beta_th_cauchy_rep;
    for (cna in 1:n_cna){
      lambda_alpha_th_cauchy_rep[cna] = mu_clock * couchy_cdf(size(idx_cauchy[c]), location_c, scales_th_cauchy[c], t_eca, t_cna[cna]);
      lambda_beta_th_cauchy_rep[cna] = mu_clock * couchy_cdf(size(idx_cauchy[c]), location_c, scales_th_cauchy[c], t_cna[cna], t_mrca);
    }
  }
  ...
  for (cna in 1:n_cna){
    m_alpha_rep[cna] = poisson_rng(l_CNA[cna]*omega*(lambda_alpha_clock + lambda_alpha_th_step + lambda_alpha_th_cauchy));
    m_beta_rep[cna] = poisson_rng(l_CNA[cna]*omega*coeff*(lambda_beta_clock + lambda_beta_th_step + lambda_beta_th_cauchy));
  }
