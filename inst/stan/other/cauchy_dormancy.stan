...
data{
  ...
  int <lower = 0> m_clock;
	...
	real <lower =0> cauchy_centre;
	real <lower=0> cauchy_centre_alpha;
	real <lower=0> cauchy_centre_beta;
}
...
model{
  ...
  th2_diff ~ gamma(cauchy_centre_alpha, cauchy_centre_beta);
  ...
  - couchy_cdf( cauchy_centre , th2_diff, t_dorm_end, t_mrca) ));
  ...
  m_mutag ~  poisson(2*mu*omega*diploid_length*couchy_cdf(cauchy_centre,th2_diff, t_dorm_end, t_mrca));

  m_alpha ~  poisson(omega*CNA_length*(

    mu*((t_cn_tr - t_eca)
      - lambda_therapy(t_eca,t_cn_tr,Therapy_1_start - delta(Therapy_1_start,t_dorm_start,t_dorm_end,k_sm),Therapy_1_end - delta(Therapy_1_end,t_dorm_start,t_dorm_end,k_sm),k)
      - couchy_cdf( cauchy_centre - delta(cauchy_centre,t_dorm_start,t_dorm_end,k_sm), th2_diff, t_eca, t_cn_tr))

      + mu_th_1*lambda_therapy(t_eca,t_cn_tr,Therapy_1_start - delta(Therapy_1_start,t_dorm_start,t_dorm_end,k_sm), Therapy_1_end - delta(Therapy_1_end,t_dorm_start,t_dorm_end,k_sm), k)
      //+ mu_th_1*couchy_cdf(cauchy_centre - delta(cauchy_centre,t_dorm_start,t_dorm_end,k_sm), th2_diff, t_eca, t_cn_tr)));
      + mu*couchy_cdf(cauchy_centre - delta(cauchy_centre,t_dorm_start,t_dorm_end,k_sm), th2_diff, t_eca, t_cn_tr)));

  m_beta ~ poisson(coeff*omega*CNA_length*(

    mu*(t_mrca - (t_dorm_end - t_dorm_start) - t_cn_tr
    - lambda_therapy(t_cn_tr,t_mrca-(t_dorm_end - t_dorm_start), Therapy_1_start - delta(Therapy_1_start,t_dorm_start,t_dorm_end,k_sm), Therapy_1_end - delta(Therapy_1_end,t_dorm_start,t_dorm_end,k_sm),k)
    - couchy_cdf(cauchy_centre - delta(cauchy_centre,t_dorm_start,t_dorm_end,k_sm), th2_diff, t_cn_tr, t_mrca))

  + mu_th_1*lambda_therapy(t_cn_tr,t_mrca-(t_dorm_end - t_dorm_start), Therapy_1_start - delta(Therapy_1_start,t_dorm_start,t_dorm_end,k_sm), Therapy_1_end - delta(Therapy_1_end,t_dorm_start,t_dorm_end,k_sm),k)
  //+ mu_th_1*couchy_cdf(cauchy_centre - delta(cauchy_centre,t_dorm_start,t_dorm_end,k_sm), th2_diff, t_cn_tr, t_mrca)));
  + mu*couchy_cdf(cauchy_centre - delta(cauchy_centre,t_dorm_start,t_dorm_end,k_sm), th2_diff, t_cn_tr, t_mrca)));
...
generated quantities{
  ...
  + mu*couchy_cdf(cauchy_centre - delta(cauchy_centre,t_dorm_start,t_dorm_end,k_sm), th2_diff, t_cn_tr, t_mrca)));
    ...
    int m_clock_rep =  poisson_rng(2*mu*omega*diploid_length*((Chemo_start - t_eca) + (t_mrca-t_dorm_end)
    - lambda_therapy(t_dorm_end, t_mrca, Therapy_1_start, Therapy_1_end,k)
     - couchy_cdf( cauchy_centre , th2_diff, t_dorm_end, t_mrca) ));
    ...
    //int m_th_1_rep =  poisson_rng(2*mu_th_1*omega*diploid_length*couchy_cdf(cauchy_centre,th2_diff, t_dorm_end, t_mrca));
    int m_mutag_rep =  poisson_rng(2*mu*omega*diploid_length*couchy_cdf(cauchy_centre,th2_diff, t_dorm_end, t_mrca));
    t_copynumber = traslation(t_cn_tr,  t_dorm_start,  t_dorm_end );
