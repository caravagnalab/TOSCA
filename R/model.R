...
generate_data = function(x){
  ...
  paste(line1, mutations_lines, parameters_lines, clinical_times_lines, linef, collapse='\n')
}

generate_parameters = function(x){
...
generate_tranfsormed_parameters = function(x){}
...
generate_model = function(x){
  ...
  paste(line1, tECA_line, tMRCA_line, tCNA_line, omega_line, mu_th_line,
        m_clock_line, m_alpha_line, m_beta_line, m_th_lines, penalty_line, linef, collapse = '\n')
}

generate_generated_quantities = function(x){}

generate_stan_code = function(x, saving_dir = NULL){
  ...
  writeLines(stan_code, paste0(saving_dir,"/stan_model.txt"))
  return(stan_code)
}
